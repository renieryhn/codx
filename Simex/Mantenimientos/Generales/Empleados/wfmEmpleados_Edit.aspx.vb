Imports prjDatos
Public Class wfmEmpleados_Edit
    Inherits System.Web.UI.Page
    Private Funciones As New funciones
    Private dtDatos As New DataTable
    Private sMensaje As String = ""
    Private Const spNombre As String = "pEmpleados_Edit"
    Private Const spListNombre As String = "pEmpleados_List"
    Private pageTitle = "Editar Empleados"
#Region "Eventos"
...
#Region "Metodos"
    Private Sub CargarValores()
        Try
            If Session("ID") <> "" Then
                ...
                cboTratamiento.Value = dtDatos.Rows(0).Item(cboTratamiento.FieldName).ToString
                ...
                cboPuesto.Value = dtDatos.Rows(0).Item(cboPuesto.FieldName).ToString
                cboProfesion.Value = dtDatos.Rows(0).Item(cboProfesion.FieldName).ToString
                cboOficina.Value = dtDatos.Rows(0).Item(cboOficina.FieldName).ToString
    ...
    Public Function dtParametros(Optional ByVal bInactivo As Boolean = False) As DataTable
        Try
            ...
            dtParametros.Rows.Add(cboTratamiento.FieldName, cboTratamiento.Value)
            dtParametros.Rows.Add(cboMunicipio.FieldName, cboMunicipio.Value)
            dtParametros.Rows.Add(cboPuesto.FieldName, cboPuesto.Value)
            dtParametros.Rows.Add(cboProfesion.FieldName, cboProfesion.Value)
