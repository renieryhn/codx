Imports prjDatos
Public Class wfmAcuerdoEdicto_List
    Inherits System.Web.UI.Page
    Dim cFunciones As New funciones
    Private sMensaje As String = ""
    Private Codigo As String = ""
    Dim Ok As Boolean
    Private sNombreBusqueda As String = "pAcuerdoEdicto_List"
    Private sNombreBorrado As String = "pAcuerdoEdicto_Delete"
    Private sNombreMantenimiento = "Mantenimiento de Acuerdos de Edicto"
...
#Region "Eventos"
    ...
    Protected Sub btnExportar_Click(ByVal sender As Object, ByVal e As EventArgs)
        Master.Exportar(GridView1, dtParametrosExportar, "Mantenimiento de Acuerdos de Edicto")
    End Sub
    ...
    Protected Sub btnNuevo_Click(ByVal sender As Object, ByVal e As EventArgs)
        If cFunciones.Filldatatable(sNombreBusqueda, dtParametrosVerificar).Rows.Count > 0 Then
            Mensaje("Ya existe un número de acuerdo correspondiente al dia de hoy")
        Else
...
#Region "Metodos"
    ...
    Private Function dtParametrosExportar() As DataTable
        Try
            ...
            dtParametrosExportar.Rows.Add("Numero de acuerdo de edicto", txtNumeroAcuerdo.Text)
    ...
    Private Function dtParametros() As DataTable
        Try
            ...
            dtParametros.Rows.Add(txtNumeroAcuerdo.FieldName, txtNumeroAcuerdo.Text)
            dtParametros.Rows.Add(dFechaRegistroDesde.FieldName, dFechaRegistroDesde.Value)
            dtParametros.Rows.Add(dFechaRegistroHasta.FieldName, dFechaRegistroHasta.Value)
        Catch ex As Exception
            Throw (ex)
        End Try
    End Function
    Private Function dtParametros(ByVal id As String) As DataTable
        ...
        End Try
    End Function
    Private Function dtParametrosVerificar() As DataTable
        Dim fecha As Date = Date.Now
        Try
            dtParametrosVerificar = cFunciones.dtDatos.Clone
            dtParametrosVerificar.Rows.Add("FechaAcuerdoEdictoDesde", fecha.ToShortDateString)
            dtParametrosVerificar.Rows.Add("FechaAcuerdoEdictoHasta", fecha.ToShortDateString)
        Catch ex As Exception
            Throw (ex)
        End Try
    End Function
